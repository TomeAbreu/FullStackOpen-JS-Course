...
const parseArgumentsExerciseCalculator = (
  ...
  const secondArgument: Array<number> = JSON.parse(args[3]);
...
const calculateExercises = (
  ...
  const trainingDays = dailyHours.reduce((accumulator, element) => {
  ...
  const ratingDescription = ratingDescriptionFunc(rating);

  const result: IExerciseCalculator = {
...
try {
  ...
  if (error.message) {
    const errorMessage = 'Something bad happened. ' + error.message;
